...
use super::{
    ...
    literal::string_literal_str,
    whitespace::{some_whitespace, wsc},
};
...
pub fn annotation(input: &str) -> IResult<&str, Annotation> {
    ...
    let (input, parameters_str) = if input.starts_with('(') {
        delimited(tag("("), get_parameters_string, tag(")"))(input)?
    } else {
        (input, "")
    };
    ...
    Ok((
        ...
        Annotation {
            name: identifier,
            parameters: parameters_str.to_string(),
        },
    ))
}
...
fn get_parameters_string(input: &str) -> IResult<&str, &str> {
...
mod tests {
    ...
    fn annotations0_no_annotations_does_not_remove_whitespace() {
        assert_eq!(annotations0(" \n"), Ok((" \n", vec![])));
    }

    #[test]
    fn annotations0_no_annotations_retuns_empty_vec() {
        assert_eq!(annotations0(" class"), Ok((" class", vec![])));
    }

    #[test]
    fn annotations0_one_annotation() {
        assert_eq!(annotations0("@a "), Ok(("", vec![a("a")])));
    }
    ...
    fn parameters_string_end_parenthesis_works() {
        assert_eq!(get_parameters_string(")"), Ok((")", "")));
    }
