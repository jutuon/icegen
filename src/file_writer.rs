...
pub fn update_generated_code_for_parsed_files(files: ParsedDartFiles) -> Result<()> {
    for file in files.code_files {
        ...
        update_generated_code_if_needed(file, generated_code)?;
    }

    Ok(())
}

pub fn update_generated_code_if_needed(file: DartFile, new_generated_code: String) -> Result<()> {
    ...
    if generated_code_path.exists() {
        ...
        if !current_generated_code.starts_with(GENERATED_FILE_HEADER) {
            return Err(anyhow!(
